<template>
  <div ref="singleProduct" class="singleProduct">
    <div class="singleProductWrapper">
      <div id="allDescs">
        <div class="status">
          <div v-if="status=='true'" class="accept"><p class="accept">تایید شده</p></div>
          <div v-else class="pending"><p class="pending">در انتظار تایید</p></div>
          <!--        <p>دو روز پیش</p>-->
          <div class="dots" @click="openToolTip($event)">
            ...
            <div class="tooltip">
              <ul>
                <li><a :href="'/users/profile/my_products/'+id+'/edit'">ویرایش</a></li>
        ...
        <div class="descs">
          <a :href="'/products/product-detial/'+slug"><p>{{ descs }}</p></a>
        </div>
        <div class="sees">
          <p>بازدید:{{sees}}</p>
        </div>
        <div class="cat">
          <p>دسته بندی:<a :href="'/categories/'+cat">{{cat}}</a></p>
        </div>
      </div>
      <div id="productImage">
    ...
    <notf @deleteProduct="deleteProduct" @closeNotf="showNotf=false" v-if="showNotf"></notf>
  </div>
</template>
...
export default {
  props:['status','ago','descs','sees','cat','id','slug','product_image'],
name: "singleProduct",
  components:{
    notf
  },
  ...
  methods:{
    deleteProduct(){
      ...
      axios.get(`/products-api/product/${this.id}/delete?csrfmiddlewaretoken=${csrf}`)
      .then(res=>{
