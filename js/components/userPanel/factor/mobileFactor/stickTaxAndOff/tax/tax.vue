    ...
    <div id="taxWrapper">
      ...
      <div class="taxOptionsWrapper">
        <div class="taxOptionWrapper">
          <input type="text" id="taxTitle" v-model="$store.state.factoreItems.tax.title">
        ...
        <div class="taxOptionWrapper">
          ...
          <span>%</span>
          <input ref="taxInput" type="text" id="taxValue" v-model="$store.state.factoreItems.tax.mount">
        ...
        <div class="taxOptionWrapper">
          <input @change="changeState($event)" ref="taxInput" type="checkbox" v-model="$store.state.factoreItems.tax.affect" id="taxValueَAffect">
...
<script>
import arrow from "../off/arrow.vue"
export default {
  name: "tax",
  data(){
    return{
  ...
  watch:{
    tax:function(value){
      this.$store.state.taxOptions.mount=value
      if(value>100){
        this.tax=100
      }
      if(value<0){
    ...
    title:function(value){
      this.$store.state.taxOptions.title=value
    }
  },
  components:{
    arrow
  ...
  methods:{
    changeState(e){
      const el=e.target
      this.$store.state.factoreItems.tax.affect=el.checked
    },
    falseThem(){
    ...
    increaseNow(){
      ...
      const interVal=setInterval(()=>{
        ...
        if(store.state.factoreItems.tax.mount==100){
          return
        }else{
          store.state.factoreItems.tax.mount++
        }
      },100)
    },
    decreaseNow(){
      ...
      const interVal=setInterval(()=>{
        ...
        if(store.state.factoreItems.tax.mount==0){
          return
        }else{
          store.state.factoreItems.tax.mount--
        }
