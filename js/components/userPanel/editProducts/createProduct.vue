      ...
      <form action="" method="post" id="createProductForm" enctype="multipart/form-data">
        ...
        >
          <tab-content title="دسته بندی و نام">
            <select-category :product="JSON.parse(product)" :cats="JSON.parse(cats)"></select-category>
            <div id="name" class="productSection">
              <div id="nameWrapper">
                <label for="">نام محصول:</label>
                  <template v-if="JSON.parse(product).title!=='None'">
          ...
          >
            ...
            <slicer></slicer>
            <aditional-information :product="JSON.parse(this.product)"></aditional-information>
            <slicer></slicer>
