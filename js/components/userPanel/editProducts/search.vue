...
export default {
  name: "search",
  props: ['items','level','product','onchange'],
  data() {
    return {
      shouldShow: null,
      selectedItem:null
  ...
  mounted() {
    ...
    if(this.level==1){
      if(this.product.category[0].main_name.mother!="None"){
        let selected=this.product.category[0].main_name.mother.split(",")
        selected=selected[0]
        selected=selected.split(":")
        selected=selected[1]

        if(firstTime==="true"){
          this.$refs.input.value=selected
        this.selectedItem=selected
        }

        let id=this.shouldShow.findIndex(i=>{
          return i.title.trim()==this.selectedItem.trim()
    ...
    if(this.level==2){
      if(this.product.category[0].main_name.title!="None"){
        const inter=setInterval(()=>{
          this.shouldShow=this.items
          if(this.shouldShow.length>0){
            if(firstTime==="true"){
            this.$refs.input.value=this.product.category[0].main_name.title

            this.selectedItem=this.product.category[0].main_name.title
            }
            this.shouldShow=this.items
            let id=this.shouldShow.findIndex(i=>{
              return i.title.trim()==this.selectedItem.trim()
    ...
    if(this.level==3){
      if(this.product.category[0].title!=="None")
      {
        const inter=setTimeout(()=>{
          this.shouldShow=this.items
          if(this.shouldShow.length>0){
            if(firstTime==="true"){
            this.$refs.input.value=this.product.category[0].title

            this.selectedItem=this.product.category[0].title
            }
