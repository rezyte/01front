<template>
  <div id="productDescs" class="productSection">
    ...
    <div id="productDescsWrapper" class="hiddenAtDisPlay">
      <textarea v-html="product.description==='None' ? '' : product.description" id="editor" name="editor"></textarea>
    </div>
  </div>
</template>
  ...
  methods: {
    init() {
      tinymce.init({
        content_style:
            `
            body * { font-family: 'iranSans' !important;font-size:16px !important;font-weight:normal;color:black;background-color:transparent }
        ...
        plugins: "image code table lists link paste",
          oninit : "setPlainText",
        toolbar:
          "undo redo | link image | code forecolor backcolor numlist bullist alignment bold | table",
