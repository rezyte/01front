        ...
        <ul>
          <template v-for="(c,i) in getAllcats">
            <li v-if="i<11" class="parentLi">
              ...
              <a :href="'/categories/'+c.slug"><h4>{{ c.title }} </h4></a>
              <div class="sideSliderSubmenu">
                <div class="subTitleWrapper">
                  <div class="SubTitle"><h4>{{ c.title }}</h4></div>
                  <ul>
                    <li v-if="subI<=5" v-for="(sub,subI) in c.subs" :key="subI"><h3><a
                        :href="getUrl(sub.slug)">{{ sub.title }}</a></h3></li>
                  </ul>
                </div>
              </div>
            </li>
  ...
  methods: {
    ...
    findNum() {
      ...
      const inter = setInterval(() => {
        const wrapper = document.getElementsByClassName("swiper-container-topSite")
